...
func RegisterUser(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
        ...
        if err := validate.Struct(user); err != nil {
            // Check if the validation error is specifically about the password length
            if validationErrors, ok := err.(validator.ValidationErrors); ok {
                for _, fieldErr := range validationErrors {
                    if fieldErr.Field() == "Password" && fieldErr.Tag() == "min" {
                        c.JSON(http.StatusBadRequest, gin.H{"error": "Password too small. Must be at least 6 characters."})
                        return
                    }
                }
            }
